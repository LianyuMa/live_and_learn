...
class TodoList extends React.Component {
  ...
  deleteTodo(title) {
    let newData = this.state.filter((todo) => (todo.title !== title));
    this.setState({ data: newData });
  }

  render() {
    const todo = this.state.data
      .map((obj) => (<Todo title={obj.title} key={obj.title}>{obj.detail}</Todo>));
    return (
...
class Todo extends React.Component {
  ...
  _onDelete() {
    this.props._onDelete(this.props.title);
  }
...
Todo.propTypes = {
  ...
  _onDelete: React.PropTypes.func.isRequired,
};
